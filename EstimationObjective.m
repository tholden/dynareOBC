function TwoNLogLikelihood = EstimationObjective( p, M_Internal, options_, oo_Internal, dynareOBC_ )
    ...
    for t = 1:dynareOBC_.EstimationFixedPointMaxIterations
        [ Mean, RootCovariance ] = KalmanStep( nan( 1, N ), OldMean, OldRootCovariance, RootQ, RootMEVar, M_Internal, options_, oo_Internal, dynareOBC_, OriginalVarSelect, LagIndices, CurrentIndices, FutureValues, NanShock );
    ...
    TwoNLogLikelihood = 0;
    for t = 1:T
        [ Mean, RootCovariance, TwoNLogObservationLikelihood ] = KalmanStep( dynareOBC_.EstimationData( t, : ), OldMean, OldRootCovariance, RootQ, RootMEVar, M_Internal, options_, oo_Internal, dynareOBC_, OriginalVarSelect, LagIndices, CurrentIndices, FutureValues, NanShock );
        OldMean = Mean;
...
function [ Mean, RootCovariance, TwoNLogObservationLikelihood ] = KalmanStep( Measurement, OldMean, OldRootCovariance, RootQ, RootMEVar, M_Internal, options_, oo_Internal, dynareOBC_, OriginalVarSelect, LagIndices, CurrentIndices, FutureValues, NanShock )
    NEndo = M_Internal.endo_nbr;
    NExo = dynareOBC_.OriginalNumVarExo;
    Nm = length( OldMean );
    Nx = Nm + NExo;
    ...
    parfor i = 1 : Mx
        InitialFullState = GetFullStateStruct( StateCubaturePoints( 1:Nm, i ), NEndo, dynareOBC_.Order, dynareOBC_.Constant ); %#ok<*PFBNS>
        ...
        if dynareOBC_.Order == 1
            NewStatePoints( :, i ) = [ Simulation.first; Simulation.bound ];
        elseif dynareOBC_.Order == 2
            NewStatePoints( :, i ) = [ Simulation.first; Simulation.second; Simulation.bound ];
        else
            NewStatePoints( :, i ) = [ Simulation.first; Simulation.second; Simulation.third; Simulation.first_sigma_2; Simulation.bound ];
        end
    end
    ...
    if No > 0
        ...
        InitialFullState = GetFullStateStruct( OldMean, NEndo, dynareOBC_.Order, dynareOBC_.Constant );
        ...
        parfor i = 1 : Mx
            Simulation = GetFullStateStruct( MeasurementCubaturePoints( 1:Nm, i ), NEndo, dynareOBC_.Order, dynareOBC_.Constant );
