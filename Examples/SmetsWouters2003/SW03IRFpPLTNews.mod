...
r = max( -0.0150009926775465 * 100, rT + 0.1 * p );
...
ms       =   cscaleem*em(-@{Horizon}); //Monetary policy innovation
qs       =   -cscaleqs*eqs;
...
stoch_simul( order=1, irf=40, periods=0, irf_shocks = ( em ) ) yobs cobs lobs piobs pobs robs;
