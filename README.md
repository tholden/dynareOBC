...
* **For controlling and performing model diagnostics**
    ...
    * `LPSolver=STRING` (default: `xpress,gurobi,cplex,mosek,scip,linprog,glpk,lpsolve,cdd,qsopt,*`)
...
* **For controlling IRFs**
    * `SlowIRFs`
      Calculates a more accurate approximation to expected IRFs using Monte-Carlo simulation. Without this option, DynareOBC calculates expected IRFs via cubature (unless this is also disabled). We advise that `SlowIRFs` is always used for IRFs in final paper versions.
    * `IRFsAroundZero`
